...
const controls = document.querySelector('.hero-card .controls');
...
card.hidden = true;
card.innerHTML = '<strong></strong><p></p><div class="alien-swarm" aria-label="Friendly aliens">👽 👽 👽 👽 👽</div>';
document.body.appendChild(card);

const translateButton = document.createElement('button');
translateButton.id = 'translateButton';
translateButton.type = 'button';
translateButton.textContent = 'Use Translator Badge';
...
function render() {
  ...
  if (planet.textContent === 'Launchpad' && current === 'needs-translator') state('at-supply');
  ...
  translateButton.hidden = !(here && eva && hasTranslator && now !== 'resolved-eva' && now !== 'complete');
  if (!encounter) return;
  ...
  card.querySelector('p').textContent = resolved
    ...
    : hasTranslator ? 'Exit the rocket and use the Translator Badge. No weapons needed.'
...
translateButton.addEventListener('click', () => {
  if (planet.textContent !== 'Gherkin-7' || mode.textContent !== 'Astronaut EVA' || document.body.dataset.translatorBadge !== 'acquired') return;
  state('resolved-eva');
  render();
});
