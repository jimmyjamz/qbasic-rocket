...
hub.innerHTML = `
  ...
  <section aria-labelledby="stationSupplyHeading">
    <h2 id="stationSupplyHeading">Supply stop <span>Coming later</span></h2>
    <p>Future first-contact supplies will be here. Nothing to collect yet.</p>
  </section>`;
...
function renderHub() {
  const atStation = mode.textContent === 'Rocket' && document.querySelector('#planetName').textContent === 'Launchpad';
...
rewardObserver.observe(document.body, { attributes: true, attributeFilter: ['data-session-rescues', 'data-session-badges', 'data-rescue-npc-state'] });
renderHub();
