...
export function createSurfaceAdventureView(createAstronaut, level = SPROUT_LEVEL) {
  ...
  return {
    ...
    update(run) {
      ...
      if (theft) {
        const time = performance.now() * 0.001;
        const progress = run.theftProgress ?? 0;
        const dash = THREE.MathUtils.smoothstep(progress, 0, 0.35);
        ...
        thiefCrew.children.forEach((alien, index) => {
          ...
          alien.position.y = hop + (dash > 0.94 ? 0.72 + index * 0.14 : 0);
          alien.rotation.z = Math.sin(time * 7.3 + index) * 0.18;
          alien.visible = progress < 0.55;
        });
